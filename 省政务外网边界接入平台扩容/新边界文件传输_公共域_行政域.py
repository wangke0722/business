...
try:
    file_dir="C:\FTPNEW\iftp"
    dir_list=os.listdir(file_dir)
    for cur_file in dir_list:
        path=os.path.join(file_dir,cur_file)
        if os.path.isfile(path):
            if format(cur_file).startswith("202"):
                if len(cur_file)==1:
                    aaa=format(cur_file)
                elif len(cur_file)==0:
                    print("111111111")
                    ...
                    print("222222222222")
    print(aaa)
    modifiedTime = time.strftime('%Y%m%d%H%M%S', time.localtime(os.stat("C:\FTPNEW\iftp/"+aaa).st_mtime))
    f = open('C:\FTPNEW\iftp/'+aaa)
    readTime = f.read()
    f.close()
    print(modifiedTime)
