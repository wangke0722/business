...
if Element(loc = '//*[@id="userId"]',l_type = 'XPATH').wait():
    ...
    if Element(loc = '/html/body/section/section/section/aside/div/div[1]/div/ul/li[1]/div/span',l_type = 'XPATH').wait():
        ...
        #点击收文管理
        if Element(loc = '/html/body/div/ul/div/div/div[1]/ul/li[2]/a',l_type = 'XPATH').wait():
            starttime=time.time()
            #偶尔点击不到收件箱，调用js点击
            #click_link=driver.find_element_by_link_text('公文传送')
            js=driver.find_element_by_xpath('/html/body/div/ul/div/div/div[1]/ul/li[2]/a')
        ...
        sleep(10)
        chain=ActionChains(driver)
        BBB=driver.find_element_by_xpath('/html/body/section/section/section/aside/div/div[1]/div/ul/li[1]/div/span')
        chain.move_to_element(BBB).perform()
        #设置检查点
        if Element(loc = '/html/body/section/section/section/main/div/div[2]/div/div[2]/table/thead/tr/th[6]/div',l_type = 'XPATH').wait():
            #获取检查点 来文单位
            checkpoint = Element(loc = '/html/body/section/section/section/main/div/div[2]/div/div[2]/table/thead/tr/th[6]/div',l_type = 'XPATH').value()
            #输出检查点 标题
